...
{
    ImGuiTableFlags final_flags = flags;

    if ((final_flags & ImGuiTableFlags_SizingMask_) == 0)
        final_flags |= ((final_flags & ImGuiTableFlags_ScrollX) || (outer_window->Flags & ImGuiWindowFlags_AlwaysAutoResize)) ? ImGuiTableFlags_SizingFixedFit : ImGuiTableFlags_SizingStretchSame;

    if ((final_flags & ImGuiTableFlags_SizingMask_) == ImGuiTableFlags_SizingFixedSame)
        final_flags |= ImGuiTableFlags_NoKeepColumnsVisible;

    if (final_flags & ImGuiTableFlags_Resizable)
        final_flags |= ImGuiTableFlags_BordersInnerV;

    if (final_flags & (ImGuiTableFlags_ScrollX | ImGuiTableFlags_ScrollY))
        final_flags &= ~(ImGuiTableFlags_NoHostExtendX | ImGuiTableFlags_NoHostExtendY);

    if (final_flags & ImGuiTableFlags_NoBordersInBodyUntilResize)
        final_flags &= ~ImGuiTableFlags_NoBordersInBody;

    if ((final_flags & (ImGuiTableFlags_Resizable | ImGuiTableFlags_Hideable | ImGuiTableFlags_Reorderable | ImGuiTableFlags_Sortable)) == 0)
        final_flags |= ImGuiTableFlags_NoSavedSettings;
...
void ImGui::TableSetupColumn(const char* label, ImGuiTableColumnFlags flags, float init_width_or_weight, ImGuiID user_id)
{
    ...
    ImGuiTableColumnFlags column_flags = flags;

    ImGuiTableColumn* column = &table->Columns[table->DeclColumnsCount];
    table->DeclColumnsCount++;

    if (table->IsDefaultSizingPolicy && (column_flags & ImGuiTableColumnFlags_WidthMask_) == 0 && (column_flags & ImGuiTableFlags_ScrollX) == 0)
    ...
    if ((column_flags & ImGuiTableColumnFlags_WidthMask_) == 0 && init_width_or_weight > 0.0f)
    ...
    {
        column_flags |= ImGuiTableColumnFlags_NoHeaderLabel;
        table->AngledHeadersCount++;
    }

    TableSetupColumnFlags(table, column, column_flags);
    column->UserID = user_id;

    const ImGuiTableColumnFlags final_flags = column->Flags;
