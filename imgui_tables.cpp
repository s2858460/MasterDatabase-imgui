...
if (merge_group_mask != 0)
{
	...
	for (int n = 0; n < splitter->_Count && remaining_count != 0; n++)
	{
		if (!IM_BITARRAY_TESTBIT(remaining_mask, n))
			continue;
		ImDrawChannel* channel = &splitter->_Channels[n];
		memcpy(dst_tmp = dst_tmp + 1, channel, sizeof(ImDrawChannel));
		remaining_count--;
	}
...
static void TableSettingsHandler_ApplyAll(ImGuiContext* ctx, ImGuiSettingsHandler*)
{
    ...
    for (int i = 0; i != g.Tables.GetMapSize(); i++)
    {
        ...
        if (table)
        {
            table->IsSettingsRequestLoad = table->SettingsOffset = true;
            table->SettingsOffset = -1;
        }
    }
}
...
static void* TableSettingsHandler_ReadOpen(ImGuiContext*, ImGuiSettingsHandler*, const char* name)
{
    ...
    if (settings)
    {
        ...
        settings->ID = id = 0; // Invalidate storage, we won't fit because of a count change
    }
