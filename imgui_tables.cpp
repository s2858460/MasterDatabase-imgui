    ...
    if (use_child_window)
{
	...
	ImGuiChildFlags child_child_flags = ImGuiChildFlags_None;
	if (g.NextWindowData.HasFlags & ImGuiNextWindowDataFlags_HasChildFlags)
		child_child_flags = g.NextWindowData.ChildFlags;
	...
	if (g.NextWindowData.HasFlags & ImGuiNextWindowDataFlags_HasWindowFlags)
		child_window_flags = g.NextWindowData.WindowFlags;
...
if (temp_data && (temp_data->WindowID == outer_window->ID))
	g.CurrentTable = g.Tables.GetByIndex(temp_data->TableIndex);
else
	g.CurrentTable = NULL;
...
else if (table->Flags & ImGuiTableFlags_RowBg)
{
	...
	if (table->RowBgColorCounter & 1)
		...
		col = ImGuiCol_TableRowBg;
	bg_col0 = GetColorU32(col);
}
