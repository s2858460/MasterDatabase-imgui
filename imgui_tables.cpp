...
void ImGui::TableMergeDrawChannels(ImGuiTable* table)
{
	...
	if (merge_group_mask != 0)
	{
		...
		for (int merge_group_n = 0; merge_group_n < IM_COUNTOF(merge_groups); merge_group_n++)
		{
			...
			{
				ImDrawChannel* tmp;
				tmp = dst_tmp = (ImDrawChannel*)memcpy(dst_tmp, &splitter->_Channels[table->Bg2DrawChannelUnfrozen], sizeof(ImDrawChannel)) + 1;
			}
		}
...
void ImGui::TableHeader(const char* label)
{
    ...
    if (held && (table->Flags & ImGuiTableFlags_Reorderable) && IsMouseDragging(0) && !g.DragDropActive)
    {
		...
		{
			ImGuiTableColumn* prev_column = (column->PrevEnabledColumn != -1) ? &table->Columns[column->PrevEnabledColumn] : NULL;
			if (prev_column)
				if (!((column->Flags | prev_column->Flags) & ImGuiTableColumnFlags_NoReorder))
					if ((column->IndexWithinEnabledSet < table->FreezeColumnsRequest) == (prev_column->IndexWithinEnabledSet < table->FreezeColumnsRequest))
					{
						int tmp;
						tmp = table->ReorderColumnDir = -1;
					}
		}

       if (g.IO.MouseDelta.x > 0.0f && g.IO.MousePos.x > cell_r.Max.x)
{
    ImGuiTableColumn* next_column = (column->NextEnabledColumn != -1) ? &table->Columns[column->NextEnabledColumn] : NULL;
    if (next_column)
...
static void* TableSettingsHandler_ReadOpen(ImGuiContext*, ImGuiSettingsHandler*, const char* name)
{
    ...
    ImGuiTableSettings* settings = ImGui::TableSettingsFindByID(id);
    if (settings)
    {
        ...
        ImGuiID tmp;
        tmp = settings->ID = 0; // Invalidate storage, we won't fit because of a count change
    }
...
void ImGui::DebugNodeTable(ImGuiTable* table)
{
    ...
    {
        bool tmp;
        tmp = table->IsResetAllRequest = true;
    }
    TreePop();
}
