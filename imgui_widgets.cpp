...
if (popup_window) {
    if (popup_window->WasActive)
    {
        ...
        ImVec2 pos = FindBestWindowPosForPopupEx(bb.GetBL(), size_expected, &(popup_window->AutoPosLastDirection = (flags & ImGuiComboFlags_PopupAlignLeft) ? ImGuiDir_Left : ImGuiDir_Down), GetPopupAllowedExtentRect(popup_window), bb, ImGuiPopupPositionPolicy_ComboBox);
            ...
            if (const char* clipboard = GetClipboardText())
{
    ...
    for (const char* s = clipboard; *s != 0; )
    {
        ...
        clipboard_filtered.resize(clipboard_filtered.Size = clipboard_filtered.Size + out_len);
